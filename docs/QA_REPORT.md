...
- `.\mvnw.cmd -f pom.xml test`: cliente Swing, 18 pruebas, 0 fallos.
- `.\mvnw.cmd -f finanzas-backend\pom.xml test`: backend, 25 pruebas, 0 fallos, 1 omitida por falta de Docker local.
...
- Flyway/PostgreSQL: `FlywayPostgresMigrationTest` prueba V3 -> V4 con Testcontainers cuando Docker esta disponible.
- Cliente: regresiones de persistencia, avatar local, precision monetaria, categorias y login local.
