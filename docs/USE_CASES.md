...
| UC-002 | Inicio de sesion | Usuario / Backend | Abrir sesion con credenciales | Usuario existe | 1. Validar rate limit. 2. Verificar password. 3. Emitir access/refresh. 4. Sincronizar snapshot | Credenciales 401; bloqueo 429 | Tokens no se persisten; refresh rotado | `RefreshTokenServiceTest`, pruebas local login |
| UC-003 | Recuperacion de password | Usuario / email sink | Recuperar acceso | Email verificable | 1. Solicitar recuperacion. 2. Emitir token. 3. Enviar correo. 4. Cambiar password | Token expirado/reusado | Pendiente de implementar backend/UI | Pendiente |
| UC-004 | Perfil y avatar | Usuario / Backend | Actualizar perfil/avatar | Sesion activa | 1. Editar perfil. 2. Validar unicidad email. 3. Subir avatar multipart. 4. Backend decodifica/normaliza. 5. Cliente cachea por ETag | Avatar corrupto 422; sin avatar 404 | UserResponse no expone rutas fisicas; avatar aislado por usuario | `AvatarStorageServiceTest`; perfil local en `DataManagerRegressionTest` |
| UC-005 | Crear workspace | Usuario / Backend | Crear espacio personal/hogar/negocio | Sesion activa | 1. Capturar nombre/tipo. 2. Crear workspace. 3. Asignar OWNER | Validacion 422 | Workspace y miembro owner | Endpoints existentes; falta E2E cliente |
| UC-006 | Seleccionar workspace | Usuario / Backend | Cambiar hogar activo | Lista de workspaces fresca | 1. Listar workspaces. 2. Elegir workspace. 3. Verificar membresia. 4. Sincronizar | Workspace removido 403/404 | Pendiente: selector UI y persistencia por usuario | Pendiente |
| UC-007 | Gestion del hogar | OWNER/ADMIN / miembros | Administrar integrantes | Workspace hogar | 1. Ver miembros. 2. Invitar/remover/cambiar rol. 3. Auditar | Sin permisos 403 | Backend soporta miembros; Swing parcial | `WorkspaceAccessServiceTest`; falta UI E2E |
| UC-008 | Invitaciones | OWNER/ADMIN, invitado / Backend | Invitar, aceptar o rechazar | Email destino valido | 1. Crear invitacion. 2. Mostrar bandeja interna. 3. Aceptar/rechazar. 4. Refrescar workspaces | Expirada/cancelada 409/410 | Backend tiene endpoints; UI incompleta | Pendiente E2E dos usuarios |
...
| UC-018 | Gastos compartidos | MEMBER+ / miembros | Dividir gastos reales | Miembros UUID validos | 1. Elegir pagador. 2. Elegir participantes. 3. Calcular splits. 4. Guardar | Montos no cuadran 422; self payment bloqueado en liquidacion | Backend opera por UUID; cliente aun mapea nombres en algunos flujos | `SharedExpenseSplitCalculatorTest`; falta E2E regresion reportada |
| UC-019 | Liquidaciones | MEMBER+ / Backend | Reducir deuda pendiente | Deuda existente | 1. Elegir deudor/acreedor. 2. Validar monto. 3. Registrar settlement. 4. Recalcular balances | Monto superior 409/422 | No desactiva miembros; historial se conserva | Tests parciales backend; falta E2E |
...
| UC-025 | Ajustes, tema y moneda | Usuario / Backend/local | Persistir preferencias | Perfil activo | 1. Elegir moneda/locale. 2. Guardar. 3. Aplicar formato | Preferencia invalida | Moneda parcial; tema/backend settings pendientes | Pendiente |
