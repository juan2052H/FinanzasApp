...
| UC-003 | Recuperacion de password | Usuario / email sink | Recuperar acceso | Email verificable | 1. Solicitar recuperacion. 2. Emitir token hasheado. 3. Enviar correo a sink. 4. Cambiar password. 5. Revocar refresh tokens | Token expirado/reusado; correo desconocido no enumera | Backend y LoginFrame implementan solicitud/confirmacion; SMTP pendiente | `AccountTokenServiceTest`, `AuthApplicationServiceTest`, `FinanzasApiClientTest` |
| UC-004 | Perfil y avatar | Usuario / Backend | Actualizar perfil/avatar | Sesion activa | 1. Editar perfil. 2. Validar unicidad email. 3. Reiniciar verificacion si cambia email. 4. Subir avatar multipart. 5. Cliente cachea por ETag | Avatar corrupto 422; sin avatar 404; token verificacion expirado 410 | UserResponse no expone rutas fisicas; avatar aislado; `emailVerified` visible | `AvatarStorageServiceTest`; perfil local en `DataManagerRegressionTest`; auth lifecycle tests |
...
| UC-007 | Gestion del hogar | OWNER/ADMIN / miembros | Administrar integrantes | Workspace hogar | 1. Ver miembros. 2. Invitar/remover/cambiar rol. 3. Auditar | Sin permisos 403 | Backend soporta miembros; Swing lista/invita/remueve/cambia rol | `WorkspaceAccessServiceTest`, `WorkspaceCollaborationServiceTest`; falta UI E2E |
